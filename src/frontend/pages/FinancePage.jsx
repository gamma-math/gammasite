...
function groupAccounts(accounts) {
  ...
  return [...mainGroups.values()].map((main) => ({
    ...
    subGroups: [...main.subGroups.values()].map((sub) => ({
      ...sub,
      name: readableLabel(sub.labels) || "Uden underkonto",
    })),
  }));
}
...
function MetricCard({ label, value, negative = false }) {
  return (
    <article className="finance-live-card">
      <div className="finance-live-card-heading">
        <h2>{label}</h2>
        <span className={`finance-live-dot ${negative ? "is-negative" : ""}`} />
      </div>
      <strong className={`${amountClass(value)} finance-live-metric`}>
        {formatAmount(value)}
      </strong>
    </article>
  );
  ...
  return (
    <div className="menu-panel-header finance-live-hero">
      ...
      <div className="finance-live-hero-meta">
        <p className="finance-live-last-updated">
          Senest opdateret
          <br />
          <strong>{lastUpdated ?? "Henter..."}</strong>
        </p>
        <a
        ...
        <div className="finance-live-year-toggle" aria-label="Vælg år">
          <button
            className={year === currentYear ? "is-active" : ""}
            onClick={() => setYear(currentYear)}
          >
            Dette år
          </button>
          <button
            className={year === currentYear - 1 ? "is-active" : ""}
...
function MonthlyChart({ monthly, isCurrentYear }) {
  ...
  const values = visibleMonths.map((_, index) =>
    Number(monthly?.[index]?.net ?? 0),
  );
    ...
    <section className="finance-live-panel finance-live-chart-panel">
      <div className="finance-live-panel-heading">
        <div>
          ...
          <h2>Nettoresultat pr. måned</h2>
        </div>
        <span className="finance-live-badge">DKK</span>
      </div>
      <div
          ...
          <div className="finance-live-chart-column" key={visibleMonths[index]}>
            ...
            <div className="finance-live-chart-track">
              <span
                ...
                style={{
                  height: `${value === 0 ? 3 : Math.max(8, (Math.abs(value) / maxValue) * 100)}%`,
                }}
              />
            </div>
            <span className="finance-live-chart-label">
              {visibleMonths[index]}
            </span>
          </div>
        ))}
...
function Resultatopgørelse({ overview }) {
  ...
  const groups = useMemo(
    () => groupAccounts(overview.accounts),
    [overview.accounts],
  );
  const toggle = (key) =>
    setExpanded((current) => {
            ...
            {groups.map((main) => {
              ...
              return (
                <Fragment key={main.key}>
                    ...
                    return (
                      <Fragment key={sub.key}>
                        <tr className="finance-live-parent-row">
                          ...
                          <td className={amountClass(realized)}>
                            {formatAmount(realized)}
                          </td>
                          <td className={amountClass(budget)}>
                            {formatAmount(budget)}
                          </td>
                          <td className={amountClass(previous)}>
                        ...
                        {isExpanded &&
                          sub.rows.map((row) => (
                            <tr
                              ...
                              key={`${sub.key}-${row.accountId}`}
                            >
                              <td>{row.context}</td>
                              <td className={amountClass(row.realized)}>
                                {formatAmount(row.realized)}
                              </td>
                              <td className={amountClass(row.budget)}>
                                {formatAmount(row.budget)}
                              </td>
                              <td className={amountClass(row.previousYear)}>
                                {formatAmount(row.previousYear)}
                              </td>
                            </tr>
                          ))}
                      </Fragment>
                  ...
                  <tr className="finance-live-total-row">
                    <th>Total {main.name}</th>
                    <td className={amountClass(sumRows(mainRows, "realized"))}>
                      {formatAmount(sumRows(mainRows, "realized"))}
                    </td>
                    <td className={amountClass(sumRows(mainRows, "budget"))}>
                      {formatAmount(sumRows(mainRows, "budget"))}
                    </td>
                    <td
                      className={amountClass(sumRows(mainRows, "previousYear"))}
                    >
                      {formatAmount(sumRows(mainRows, "previousYear"))}
                    </td>
                  </tr>
                </Fragment>
              );
            })}
            <tr className="finance-live-grand-total">
              <th>Total</th>
              <td
                className={amountClass(sumRows(overview.accounts, "realized"))}
              >
                {formatAmount(sumRows(overview.accounts, "realized"))}
              </td>
              <td className={amountClass(sumRows(overview.accounts, "budget"))}>
                {formatAmount(sumRows(overview.accounts, "budget"))}
              </td>
              <td
                className={amountClass(
                  sumRows(overview.accounts, "previousYear"),
                )}
              >
                {formatAmount(sumRows(overview.accounts, "previousYear"))}
              </td>
            </tr>
          </tbody>
        </table>
...
export function FinancePage({ user }) {
  ...
  return (
    <MenuLayout active="/react/finance" isAuthenticated={user.isAuthenticated}>
      <FinanceHeader
      ...
      {overview && (
        <>
          <div className="finance-live-metrics">
            <MetricCard label="Indbetalinger" value={overview.summary.income} />
            <MetricCard
              label="Udgifter"
              value={overview.summary.expense}
              negative
            />
            <MetricCard label="Nettoresultat" value={overview.summary.net} />
          </div>
          <MonthlyChart
